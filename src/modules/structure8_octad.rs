    ...
    pub fn get_canonical_terms(&self) -> Vec<String> {
        if let Some(ref schema) = self.schema {
            ...
            vec!["1".to_string(), "2".to_string(), "3".to_string(), "4".to_string(), 
                 "5".to_string(), "6".to_string(), "7".to_string(), "8".to_string()]  // Base layer uses 1-based numbers
        }
    }
...
mod tests {
    ...
    fn test_schema_application() {
        ...
        assert_eq!(structure.schema.as_ref().unwrap().get_schema_name(), "Bennett's Octad");
    }
