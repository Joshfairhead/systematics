...
impl SystematicStructure for Dodecad {
    ...
    fn display(&self) {
        println!("=== {} ===", self.name);
        println!("Type: Dodecad ({} terms)", Self::TERM_COUNT);
        print!("Terms: ");
        for (i, term) in self.user_term_index.iter().enumerate() {
            if i > 0 { print!(" → "); }
            print!("{}", term);
...
mod tests {
    ...
    fn test_canonical_terms() {
        ...
        let canonical = dodecad.canonical_terms();
        assert_eq!(canonical.len(), 12);
        assert_eq!(canonical[0], "Autocracy");
        assert_eq!(canonical[1], "Domination");
        assert_eq!(canonical[11], "Wholeness");
    }
