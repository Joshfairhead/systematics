...
impl Undecad {
    ...
    pub fn instances_tuple(&self) -> (&str, &str, &str, &str, &str, &str, &str, &str, &str, &str, &str) {
        (&self.user_term_index[0], &self.user_term_index[1], &self.user_term_index[2], 
         ...
         &self.user_term_index[9], &self.user_term_index[10])
    }
...
impl SystematicStructure for Undecad {
    ...
    fn validate(&self) -> Result<()> {
        ...
        for i in 0..11 {
            for j in (i + 1)..11 {
                if !self.user_term_index[i].trim().is_empty() && 
                   self.user_term_index[i].trim() == self.user_term_index[j].trim() {
                    return Err(SystematicsError::StructureValidation {
                        reason: format!("Duplicate term '{}' found at positions {} and {}", 
                                       self.user_term_index[i].trim(), i + 1, j + 1),
                    });
                }
            }
        }
    ...
    fn display(&self) {
        ...
        println!("\n--- User Terms ---");
        for (i, user_term) in self.user_term_index.iter().enumerate() {
...
mod tests {
    ...
    fn test_undecad_creation() {
        ...
        assert_eq!(undecad.first_order_connectives_name(), "Phases");
    }
    ...
    fn test_trait_compliance() {
        ...
        assert_eq!(undecad.terms().len(), 11);
    }
} 