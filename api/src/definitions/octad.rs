...
impl Octad {
    ...
    /// Get user expression by index (0-7)
    /// 
    ...
    /// Get the first user expression (maps to "Smallest Significant Holon")
    pub fn first_user_expression(&self) -> &str {
        &self.user_expressions[0]
    }
    
    /// Get the second user expression (maps to "Critical Functions")
    pub fn second_user_expression(&self) -> &str {
        &self.user_expressions[1]
    }
    
    /// Get the third user expression (maps to "Supportive Platform")
    pub fn third_user_expression(&self) -> &str {
        &self.user_expressions[2]
    }
    
    /// Get the fourth user expression (maps to "Necessary Resourcing")
    pub fn fourth_user_expression(&self) -> &str {
        &self.user_expressions[3]
    }
    
    /// Get the fifth user expression (maps to "Integrative Totality")
    pub fn fifth_user_expression(&self) -> &str {
        &self.user_expressions[4]
    }
    
    /// Get the sixth user expression (maps to "Inherent Values")
    pub fn sixth_user_instance(&self) -> &str {
        &self.user_expressions[5]
    }
    
    /// Get the seventh user expression (maps to "Intrinsic Nature")
    pub fn seventh_user_instance(&self) -> &str {
        &self.user_expressions[6]
    }
    
    /// Get the eighth user expression (maps to "Organisational Modes")
    ...
    pub fn instances_tuple(&self) -> (&str, &str, &str, &str, &str, &str, &str, &str) {
        (&self.user_expressions[0], &self.user_expressions[1], &self.user_expressions[2], 
         ...
         &self.user_expressions[6], &self.user_expressions[7])
    }
...
mod tests {
    ...
    fn test_trait_compliance() {
        ...
        assert_eq!(octad.user_expressions().len(), 8);
