...
use crate::schemas::{StructureSchema, select_tetrad_schema};

pub struct TetradicStructure {
    pub name: String,
    // Positional instances (A, B, C, D)
    pub positions: [String; 4],
...
impl TetradicStructure {
    /// Creates a new TetradicStructure with empty positions
    ...
    /// Creates a new TetradicStructure with specific position values
    pub fn new_with_positions(
        ...
        let mut structure = TetradicStructure {
            name: name.to_string(),
            positions: [
                pos_a.to_string(),
                pos_b.to_string(),
    ...
    pub fn apply_schema(&mut self, schema: Box<dyn StructureSchema>) {
        if schema.get_position_count() != 4 {
            panic!("Schema must support exactly 4 positions for TetradicStructure");
        }
        self.schema = Some(schema);
        self.refresh_connectives_with_schema();
    }

    /// Initialize default positional connectives
    ...
    pub fn create_interactive() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let mut positions = [String::new(), String::new(), String::new(), String::new()];
        ...
        if modify_connectives.starts_with('y') {
            ...
            println!("Note: Tetrad has 6 connectives.");
...
impl std::fmt::Debug for TetradicStructure {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let schema_name = self.schema.as_ref()
            .map(|s| s.get_schema_name())
            .unwrap_or("No Schema");
        
        f.debug_struct("TetradicStructure")
...
mod tests {
    ...
    #[test]
    fn test_tetradic_structure_creation() {
        let structure = TetradicStructure::new_with_positions(
            "Test Tetrad",
            "Foundation", 
            "Vision",
            "Method",
        ...
        assert_eq!(structure.name, "Test Tetrad");
        assert_eq!(structure.positions[0], "Foundation");
        assert_eq!(structure.positions[1], "Vision");
        assert_eq!(structure.positions[2], "Method");
        assert_eq!(structure.positions[3], "Purpose");
        
        // Should have default connectives
        assert!(structure.has_connectives());
        assert_eq!(structure.connectives_count(), 6);
    }

    #[test]
    fn test_schema_application() {
        let mut structure = TetradicStructure::new("Test");
        let schema = Box::new(BennettTetradSchema);
        
        structure.apply_schema(schema);
        assert!(structure.schema.is_some());
        assert_eq!(structure.schema.as_ref().unwrap().get_schema_name(), "Bennett's Tetrad");
    }

    #[test]
    fn test_connective_operations() {
        let mut structure = TetradicStructure::new_with_positions(
            "Test", "A", "B", "C", "D"
        );
    ...
    fn test_schema_aware_connectives() {
        ...
        assert_eq!(structure.get_connective(0, 1).unwrap(), "Motivational imperative");
        assert_eq!(structure.get_connective(1, 2).unwrap(), "Effectual compatibility");
    }

    #[test]
    fn test_bennett_schema() {
        let mut structure = TetradicStructure::new("Bennett Test");
        structure.apply_schema(Box::new(BennettTetradSchema));
        structure.initialize_default_connectives();
        
        // Should use Bennett's schema connective labels
        assert_eq!(structure.get_connective(0, 1).unwrap(), "Motivational imperative");
        assert_eq!(structure.get_connective(0, 2).unwrap(), "Technical power");
        assert_eq!(structure.get_connective(1, 2).unwrap(), "Effectual compatibility");
    }
} 